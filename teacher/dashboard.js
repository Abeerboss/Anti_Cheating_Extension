...
async function loadOverview() {
  ...
  try {
    var [students, allLogs, session] = await Promise.all([fbReadAll("students"), fbReadAll("logs"), getSession()])
    ...
    var activeNow  = (session && session.exam_active) ? 1 : 0
    ...
    content.innerHTML =
      ...
      '<div class="stats-row">' +
        ...
        statBox("green",  activeNow,       "Active Now") +
      '</div>' +
      '<div class="stats-row" style="margin-top:-8px">' +
        statBox("red",    aiVisits,    "AI Site Visits") +
      ...
      '</div>' +
      addStudentForm() +
      panel("Registered Students", students.length + " registered", renderStudentTable(students, allLogs, session)) +
...
async function loadStudent(studentId) {
  ...
  try {
    ...
    var isActive = session && session.exam_active && session.student_id === studentId

    $("view-title").textContent = student.name
    $("view-sub").textContent   = studentId + " · " + sLogs.length + " violation" + (sLogs.length !== 1 ? "s" : "")
    content.innerHTML = renderStudentProfile(student, sLogs, isActive)
    loadSidebar()
  } catch(e) { $("main-content").innerHTML = '<div class="empty-state">Error: ' + e.message + '</div>'; console.warn("[ACE]", e) }
}
